...
export default async function FactionPage() {
  ...
  const actorAssignment = access.assignments.find((assignment) => assignment.tornUserId === actor.tornUserId && assignment.status === "ACTIVE");
  const canManage = isPlatformOwner(actor) || Boolean(actorAssignment && hasPermission(actorAssignment.role, "access:manage"));
