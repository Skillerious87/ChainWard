...
import { Activity, AlarmClock, Check, ChevronsUpDown, Clock3, Eye, Search, ShieldAlert, ShieldCheck, SlidersHorizontal, Umbrella, UserRoundCheck, UsersRound } from "lucide-react";
...
type ActivityView = "all" | "attention" | "critical" | "active" | "holiday" | "watch";
type ActivitySort = "attention" | "recent" | "name" | "tenure";
...
export function MemberActivityWorkspace({ rosterResult, telemetry, activity, canManage }: { rosterResult: TornDataResult<TornRosterMember[]>; telemetry: WorkspaceTelemetry; activity: ActivityWorkspace; canManage: boolean }) {
  ...
  return <div className="page-stack member-activity-workspace">
    <PageHeader eyebrow="Faction readiness" title="Member activity" description="Owner-grade inactivity intelligence with protected absences, escalation signals, and an auditable follow-up policy." actions={<ExportButton filename="chainward-member-activity.csv" label="Export activity" rows={exportRows} />} />
    ...
    <section className="data-section member-activity-table-section">
      <div className="section-heading member-activity-table-heading"><div><h2>Roster activity tracker</h2><p>{visible.length} of {members.length} verified members match this view</p></div><div className="table-tools"><label className="search-field"><Search size={15} /><span className="sr-only">Search member activity</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Member, position, ID, or note" /></label><label className="member-activity-select"><SlidersHorizontal size={14} /><span className="sr-only">Faction position</span><select value={position} onChange={(event) => setPosition(event.target.value)}>{positions.map((item) => <option key={item}>{item}</option>)}</select></label></div></div>
      <div className="member-activity-tabs" role="tablist" aria-label="Member activity view">{([ ["all", "All members", members.length], ["attention", "Needs review", summary.attention], ["critical", "Critical", summary.critical], ["active", "Active 24h", summary.activeDay], ["holiday", "Holiday", summary.holiday], ["watch", "Watch list", summary.watch] ] as const).map(([value, label, count]) => <button key={value} role="tab" aria-selected={view === value} className={view === value ? "member-activity-tab--active" : undefined} onClick={() => setView(value)}>{label}<span>{count}</span></button>)}</div>
      <div className="member-activity-sort"><span>Sort</span>{([ ["attention", "Priority"], ["recent", "Recent activity"], ["name", "Name"], ["tenure", "Tenure"] ] as const).map(([value, label]) => <button className={sort === value ? "member-activity-sort--active" : undefined} key={value} onClick={() => setSort(value)}>{label}{sort === value && <ChevronsUpDown size={12} />}</button>)}</div>
...
function compareRows(left: MemberActivityAssessment, right: MemberActivityAssessment, sort: ActivitySort): number {
  ...
  return right.riskScore - left.riskScore || right.ageSeconds - left.ageSeconds;
}
