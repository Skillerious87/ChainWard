      ...
      <div className="table-scroll" role="region" aria-label="Access request table" tabIndex={0}>
        <table className="data-table access-request-table">
          ...
          <tbody>{visible.map((request) => <tr key={request.requestId}>
            <td data-label="Faction"><div className="access-request-table__cell-body"><strong>{request.faction}</strong><small className="cell-subtext">Torn faction ID {request.factionId}</small></div></td>
            <td data-label="Plan"><div className="access-request-table__cell-body"><strong>{request.plan}</strong><small className="cell-subtext">{request.payment} · {request.term}</small></div></td>
            <td data-label="Submitted by"><div className="access-request-table__cell-body"><TornUserName name={request.contact.name} tornUserId={request.contact.tornUserId} /><small className="cell-subtext">Request {shortId(request.requestId)}</small></div></td>
            <td data-label="Payment ID"><div className="access-request-table__cell-body"><span className="identifier-cell"><code className="reference-code">{request.reference}</code><button onClick={() => void copyIdentifier(request.reference)} aria-label={`Copy ${request.reference}`}><Copy size={12} /></button></span><small className="cell-subtext">{new Date(request.submittedAt).toLocaleString("en-GB")}</small></div></td>
            <td data-label="Status"><div className="access-request-table__cell-body"><span className={`payout-badge payout-badge--${statusClass(request.status)}`}>{request.status === "Approved" && <Check size={12} />}{request.status}</span>{request.reviewedBy && <small className="cell-subtext">by <TornUserName name={request.reviewedBy.name} tornUserId={request.reviewedBy.tornUserId} /></small>}</div></td>
