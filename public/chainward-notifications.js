...
self.addEventListener("activate", (event) => {
  // The old opaque-response cache could preserve a CDN error indefinitely.
  // Let the browser load profile images normally and clear that legacy cache.
  event.waitUntil(Promise.all([
    self.clients.claim(),
    caches.delete("chainward-avatar-cache-v1").catch(() => false),
  ]));
});
