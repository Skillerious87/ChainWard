...
describe.sequential("local SQLite database", () => {
  ...
  it("creates a real database file with reward and settlement tables", () => {
    ...
    try {
      ...
      expect(tables.map((table) => table.name)).toEqual(expect.arrayContaining(["reward_schemes", "reward_tiers", "chain_settlements", "faction_settings", "faction_access_assignments", "faction_access_audit", "member_activity_audit", "remembered_torn_connections"]));
