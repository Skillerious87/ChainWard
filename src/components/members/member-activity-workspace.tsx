  ...
  return <div className="page-stack member-activity-workspace">
    ...
    <section id="members-panel-roster" className="member-workspace-panel" role="tabpanel" aria-labelledby="members-tab-roster" hidden={workspaceView !== "roster"}>
      <section className="data-section member-activity-table-section">
        ...
        <div className="member-roster-cards" aria-label="Roster activity cards">{pagedRows.map((row) => <MemberRosterCard key={row.member.tornId} row={row} canManage={canManage} onInspect={() => inspect(row.member)} />)}{filteredRows.length === 0 && <div className="member-roster-cards__empty"><Search size={18} /><p><strong>No matching members</strong><span>Adjust the search or activity filters to see more of the roster.</span></p></div>}</div>
...
function MemberRosterCard({ row, canManage, onInspect }: { row: MemberActivityAssessment; canManage: boolean; onInspect: () => void }) {
  return <article className={`member-roster-card member-roster-card--${rosterTone(row)}`}>
    <header><TornUserLink name={row.member.name} tornUserId={row.member.tornId} detail={`${row.member.position || "Unassigned"} · Level ${row.member.level}`} /><span className={`activity-band activity-band--${bandClass(row.band)}`}><i />{row.band}</span></header>
    <p className="member-roster-card__reason">{row.reason}</p>
    <dl>
      <div><dt>Last activity</dt><dd>{row.member.lastAction}</dd><small>{formatAge(row.ageSeconds)}</small></div>
      <div><dt>Torn status</dt><dd><span className={`member-status member-status--${statusClass(row.member.status)}`} title={row.member.statusDescription}><i />{row.member.status}</span></dd><small>{row.member.statusDescription || "No status detail"}</small></div>
    </dl>
    <div className="member-roster-card__managed"><span>Managed state</span><ManagedState row={row} /></div>
    {row.record?.note && <p className="member-roster-card__note"><strong>Manager note</strong><span>{row.record.note}</span></p>}
    <footer><span>{row.member.daysInFaction.toLocaleString()} days in faction</span><div className="member-row-actions"><Link href={`/members/${row.member.tornId}`}><FileUser size={14} /> Open report</Link><button type="button" className="row-manage-button" onClick={onInspect}>{canManage ? row.record ? "Update activity" : "Manage activity" : "Inspect activity"}</button></div></footer>
  </article>;
}
