  ...
  return <div className="page-stack member-activity-workspace">
    ...
    <section id="members-panel-roster" className="member-workspace-panel" role="tabpanel" aria-labelledby="members-tab-roster" hidden={workspaceView !== "roster"}>
      <section className="data-section member-activity-table-section">
        ...
        <div className="table-scroll member-roster-table-scroll" role="region" aria-label="Roster activity table" tabIndex={0}><table className="data-table member-activity-table"><caption className="sr-only">Faction member activity, status, and owner-managed follow-up state</caption><thead><tr><th>Member</th><th>Faction position</th><th>Last activity</th><th>Smart signal</th><th>Torn status</th><th>Managed state</th><th><span className="sr-only">Member actions</span></th></tr></thead><tbody>{pagedRows.map((row) => <tr key={row.member.tornId} className={row.critical ? "member-activity-row--critical" : row.needsAttention ? "member-activity-row--attention" : undefined}><td data-label="Member"><TornUserLink name={row.member.name} tornUserId={row.member.tornId} detail={`Level ${row.member.level}`} /></td><td data-label="Faction position">{row.member.position || <span className="muted-value">Unassigned</span>}<small className="cell-subtext">{row.member.daysInFaction.toLocaleString()} days</small></td><td data-label="Last activity"><strong>{row.member.lastAction}</strong><small className="cell-subtext">{formatAge(row.ageSeconds)}</small></td><td data-label="Smart signal"><span className={`activity-band activity-band--${bandClass(row.band)}`}><i />{row.band}</span><small className="cell-subtext member-activity-reason">{row.reason}</small></td><td data-label="Torn status"><span className={`member-status member-status--${statusClass(row.member.status)}`} title={row.member.statusDescription}><i />{row.member.status}</span></td><td data-label="Managed state"><ManagedState row={row} /></td><td data-label="Actions"><div className="member-row-actions"><Link href={`/members/${row.member.tornId}`}><FileUser size={13} /> Report</Link><button className="row-manage-button" onClick={() => inspect(row.member)}>{canManage ? row.record ? "Update" : "Manage" : "Inspect"}</button></div></td></tr>)}</tbody></table>{filteredRows.length === 0 && <div className="table-empty">No members match the selected activity view and filters.</div>}</div>
