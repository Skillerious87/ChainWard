...
import { ArrowDown, ArrowUp, Check, ChevronsUpDown, CircleDollarSign, Search, SlidersHorizontal } from "lucide-react";
...
export function ContributionTable({ members, title = "Chain contribution", compact = false, emptyMessage = "Torn has not returned contribution records for this chain.", rewards, rewardUnit, payoutStatus = null, showRewards = false, rewardMessage }: ContributionTableProps) {
  ...
  const rewardColumnVisible = showRewards || Boolean(rewards);
  ...
  return (
    <section className="data-section">
      ...
      {showRewards && !rewards && <div className="contribution-reward-notice" role="status">
      ...
      <div className="table-scroll" role="region" aria-label="Chain contribution table" tabIndex={0}>
        <table className="data-table contribution-table">
          ...
          <tbody>{renderedMembers.map((member) => <tr key={member.tornId}>
            ...
            {rewardColumnVisible && <td className="numeric reward-column" data-label="Member reward">{rewards
              ? <RewardAmount amount={rewards[member.tornId]?.amount ?? 0} unit={rewardUnit ?? "units"} detail={rewards[member.tornId]?.tierLabel ?? "No matching tier"} paid={payoutStatus === "PAID"} />
              : <span className="reward-pending"><strong>Not calculated</strong><small>Review reward setup</small></span>}</td>}
