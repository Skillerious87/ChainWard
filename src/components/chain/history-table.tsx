...
export function HistoryTable({ chains, message, checkedAt, settlements }: { chains: TornChainHistoryItem[]; message: string; checkedAt: string; settlements: Record<number, ChainSettlementSummary> }) {
  ...
  return <section className="data-section">
    ...
    })}</tbody></table><ol className="mobile-data-cards history-mobile-list" aria-label="Completed chains">{visible.map((chain) => {
      const settlement = settlements[chain.id];
      return <li key={chain.id} className="history-mobile-card">
        <header><Link className="chain-id" href={`/chains/${chain.id}`}>Chain #{chain.id}</Link>{settlement?.status === "PAID" ? <span className="history-paid"><BadgeCheck size={14} /> PAID</span> : <span className="history-unpaid">Not marked</span>}</header>
        <p><strong>{formatDate(chain.endedAt)}</strong><span>{formatTime(chain.startedAt)} → {formatTime(chain.endedAt)}</span></p>
        <dl><div><dt>Final hits</dt><dd>{chain.hits.toLocaleString()}</dd></div><div><dt>Duration</dt><dd>{formatDuration(chain.endedAt - chain.startedAt)}</dd></div><div><dt>Respect</dt><dd>{chain.respect.toFixed(2)}</dd></div><div><dt>Reward</dt><dd>{settlement ? <RewardAmount amount={settlement.totalAmount} unit={settlement.rewardUnit} detail={`${settlement.memberCount} members`} paid={settlement.status === "PAID"} size="compact" artwork="liability" /> : <span className="muted-value">Not calculated</span>}</dd></div></dl>
        <Link className="mobile-record-action" href={`/chains/${chain.id}`}>{settlement ? "Open settlement" : "Calculate rewards"}<span>→</span></Link>
      </li>;
    })}</ol>{visible.length === 0 && <div className="table-empty">{query ? "No completed chains match these filters." : message}</div>}</div>
