...
/**
 * `/user/{id}?selections=profile` — a deterministic public profile for the
 * personal Targets list. The ID seeds a stable spread of statuses so the
 * Targets workspace has attackable, hospitalised, jailed and travelling rows
 * to render offline.
 */
function targetProfileFor(id: number, now: number) {
  const bucket = id % 5;
  const state = bucket === 0 ? "Okay" : bucket === 1 ? "Hospital" : bucket === 2 ? "Traveling" : bucket === 3 ? "Jail" : "Okay";
  const lastActionAgo = 60 + (id % 90) * 37;
  const hospitalUntil = now + 300 + (id % 12) * 240;
  const jailUntil = now + 600 + (id % 20) * 300;
  return {
    id,
    name: `Target ${id}`,
    level: 15 + (id % 85),
    last_action: {
      status: lastActionAgo < 900 ? "Online" : "Offline",
      timestamp: now - lastActionAgo,
      relative: lastActionAgo < 3_600 ? `${Math.max(1, Math.floor(lastActionAgo / 60))} minutes ago` : `${Math.floor(lastActionAgo / 3_600)} hours ago`,
    },
    status: {
      description: state === "Hospital" ? "In hospital for 12 mins" : state === "Traveling" ? "Traveling to Switzerland" : state === "Jail" ? "In jail for 25 mins" : "Okay",
      details: null,
      state,
      until: state === "Hospital" ? hospitalUntil : state === "Jail" ? jailUntil : null,
      color: state === "Okay" ? "green" : state === "Traveling" ? "blue" : "red",
    },
    faction: id % 3 === 0
      ? null
