...
export function PayoutLedger({ entries, message, databaseAvailable = true, section = "all" }: PayoutLedgerProps) {
  ...
  return <>
    ...
    {section !== "overview" && <section className="data-section professional-payout-ledger payout-workspace-section">
      ...
      <div className="table-scroll" role="region" aria-label="Member payout ledger table" tabIndex={0}><table className="data-table payout-ledger-table"><caption className="sr-only">Persisted member reward decisions with eligibility, payout status, processing time, and recorder</caption><thead><tr><SortableLedgerHeader label="Member" value="MEMBER" active={sort} direction={sortDirection} onSort={changeSort} /><SortableLedgerHeader label="Chain" value="CHAIN" active={sort} direction={sortDirection} onSort={changeSort} /><th>Reward decision</th><SortableLedgerHeader label="Member reward" value="AMOUNT" active={sort} direction={sortDirection} onSort={changeSort} numeric /><th>Status</th><SortableLedgerHeader label="Timeline" value="RECENT" active={sort} direction={sortDirection} onSort={changeSort} /><th>Recorded by</th><th><span className="sr-only">Open chain</span></th></tr></thead><tbody>{visible.map((entry) => <tr key={entry.id} className={`payout-ledger-row payout-ledger-row--${statusClass(entry)}`}><td><TornUserLink className="ledger-member" name={entry.memberName} tornUserId={entry.tornUserId} detail={`Torn ID ${entry.tornUserId}`} /></td><td><Link className="chain-id" href={`/chains/${entry.chainId}`}>#{entry.chainId}</Link></td><td>{entry.tierLabel ? <span className="tier-label">{entry.tierLabel}</span> : <span className="muted-value">No saved tier</span>}</td><td className="numeric ledger-reward-cell"><RewardAmount amount={entry.amount} unit={entry.rewardUnit} paid={isPayable(entry) && entry.status === "PAID"} size="compact" /></td><td><span className={`payout-badge payout-badge--${statusClass(entry)}`}>{!isPayable(entry) ? <CircleMinus size={13} /> : entry.status === "PAID" ? <BadgeCheck size={13} /> : entry.status === "WAIVED" ? <ShieldCheck size={12} /> : <ShieldAlert size={12} />}{displayStatus(entry)}</span></td><td><LedgerTimeline entry={entry} /></td><td><PayoutRecorder value={entry.processedBy} /></td><td className="payout-ledger-open"><Link href={`/chains/${entry.chainId}`} aria-label={`Open chain ${entry.chainId}`} title={`Open chain #${entry.chainId}`}><ChevronRight size={15} /></Link></td></tr>)}</tbody></table><ol className="mobile-data-cards payout-mobile-list" aria-label="Payout records">{visible.map((entry) => <li key={entry.id} className={`payout-mobile-card payout-mobile-card--${statusClass(entry)}`}>
        <header><TornUserLink className="ledger-member" name={entry.memberName} tornUserId={entry.tornUserId} detail={`Torn ID ${entry.tornUserId}`} /><span className={`payout-badge payout-badge--${statusClass(entry)}`}>{!isPayable(entry) ? <CircleMinus size={13} /> : entry.status === "PAID" ? <BadgeCheck size={13} /> : entry.status === "WAIVED" ? <ShieldCheck size={12} /> : <ShieldAlert size={12} />}{displayStatus(entry)}</span></header>
        <div className="payout-mobile-card__decision"><span><Link className="chain-id" href={`/chains/${entry.chainId}`}>Chain #{entry.chainId}</Link>{entry.tierLabel ? <small>{entry.tierLabel}</small> : <small>No saved tier</small>}</span><RewardAmount amount={entry.amount} unit={entry.rewardUnit} paid={isPayable(entry) && entry.status === "PAID"} size="compact" /></div>
        <dl><div><dt>Timeline</dt><dd><LedgerTimeline entry={entry} /></dd></div><div><dt>Recorded by</dt><dd><PayoutRecorder value={entry.processedBy} /></dd></div></dl>
        <Link className="mobile-record-action" href={`/chains/${entry.chainId}`}>Open chain record <ChevronRight size={15} /></Link>
      </li>)}</ol>{visible.length === 0 && <div className="table-empty payout-ledger-empty"><CircleDollarSign size={22} /><div><strong>{entries.length ? "No records match this control view" : "No payout records yet"}</strong><span>{entries.length ? "Reset the filters or search a different member, chain, or recorder." : message}</span></div></div>}</div>
