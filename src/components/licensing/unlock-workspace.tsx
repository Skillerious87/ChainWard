        ...
        <div className="licence-tier-list licence-tier-list--page" role="radiogroup" aria-label="Faction access plans">
          {licensePlans.map((plan) => {
            ...
            return <button key={plan.id} role="radio" aria-checked={active} className={`licence-tier-card${active ? " licence-tier-card--selected" : ""}`} onClick={() => setSelectedPlan(plan.id)}>
              ...
              <span className="licence-tier-card__price"><span className="licence-tier-card__price-tile"><Image src="/images/DonatorPackRemasterGPT.png" alt="" width={24} height={24} unoptimized /></span><span><strong>{plan.itemQuantity}</strong><small>{pluralizeItemName(plan.itemQuantity, plan.itemName)}</small></span></span>
            </button>;
          })}
        </div>
      </section>
